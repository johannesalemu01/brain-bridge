...
export default function TeacherPanel() {
  ...
  const [overview, setOverview] = useState({
    pendingCount: 0,
    totalVerified: 0,
    recentVerifiedAt: null as string | null,
  });
  ...
  const fetchPending = async () => {
    try {
      const [pendingRes, overviewRes] = await Promise.all([
        qaApi.getPending(),
        qaApi.getTeacherOverview(),
      ]);

      setPendingQas(pendingRes.data.data.qas);
  ...
  if (!user || (user.role !== "teacher" && user.role !== "admin")) {
    return (
      <Card className="p-20 text-center text-white/50 border-white/5 bg-white/2">
  ...
  return (
    <div className="space-y-12 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-white/60">
            Review, edit, and verify AI answers to maintain high educational
            quality.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 border-emerald-500/30 bg-emerald-500/3 hover:bg-emerald-500/6 transition-colors relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <Clock className="w-20 h-20 text-white" />
          </div>
          <p className="text-emerald-400 font-black uppercase tracking-widest text-[10px] mb-3">
            Pending Verification
          </p>
          <h2 className="text-4xl text-white font-black tracking-tight">
            {overview.pendingCount || pendingQas.length}
          </h2>
        </Card>
        <Card className="p-6 border-white/5 bg-white/5 opacity-50 relative overflow-hidden">
          <p className="text-white/40 font-black uppercase tracking-widest text-[10px] mb-3">
            Total Verified
          </p>
          <h2 className="text-4xl text-white font-black tracking-tight">
        ...
        <Card className="p-6 border-white/5 bg-white/5 opacity-50 relative overflow-hidden">
          ...
          <h2 className="text-2xl text-white font-black tracking-tight">
            {overview.recentVerifiedAt
              ? new Date(overview.recentVerifiedAt).toLocaleDateString()
              : "No recent"}
          </h2>
        </Card>
      </div>
      ...
      <div className="space-y-8 mt-12">
        ...
        {pendingQas.length === 0 ? (
          <Card className="p-24 text-center border-dashed border-white/10 bg-white/2 flex flex-col items-center">
            ...
            <CardDescription className="max-w-xs mx-auto text-base">
              Great job! All AI-generated answers are verified for the public
              knowledge base.
            </CardDescription>
          </Card>
        ) : (
              ...
              >
                <div className="p-8 md:p-10 relative">
                  ...
                  <CardHeader className="p-0 mb-8">
                    <CardTitle className="text-2xl leading-tight text-white/90">
                      Student Question: {qa.question}
                    </CardTitle>
                  </CardHeader>
                  ...
                  {verifyingId === qa._id ? (
                    <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
                      <div className="space-y-3">
                        <label className="text-xs font-black uppercase tracking-widest text-amber-400 ml-1">
                          Edit AI Answer for Publication
                        </label>
                        <textarea
                      ...
                      <div className="flex gap-4 pt-2">
                        <Button
                          onClick={() => handleVerify(qa._id)}
                          variant="premium"
                          className="h-12 px-8 flex items-center gap-2 bg-linear-to-r from-emerald-500 to-teal-500 shadow-lg shadow-emerald-500/20 border-0"
                        >
                        ...
                        <Button
                          ...
                          variant="outline"
                          className="h-12 border-white/10 text-white/50"
                        >
                    ...
                    <div className="space-y-8">
                      <div className="bg-white/5 p-6 rounded-3xl border border-white/5 opacity-80 group/draft relative transition-all hover:bg-white/8 hover:opacity-100">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-amber-600/5 rounded-full blur-2xl -z-10" />
                        <div className="flex items-center gap-2 mb-4 text-amber-400/60 font-black text-[10px] uppercase tracking-widest pb-3 border-b border-white/5">
                      ...
                      >
                        Review Answer{" "}
                        <CheckCircle className="w-5 h-5 group-hover/review:scale-125 transition-transform" />
                      </Button>
                    </div>
                  )}
                </div>
