      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-white/60">
            Multilingual AI Tutoring in Amharic & English.
          </p>
        </div>
        ...
        <div className="p-8 md:p-16 flex flex-col items-center justify-center text-center relative min-h-[450px]">
          ...
          {!isRecording && !processing && (
            <div className="mb-12 animate-in fade-in zoom-in-95 duration-500">
              ...
              <p className="text-white/40 text-lg max-w-sm mx-auto">
                Tap to speak. AI will explain naturally with audio.
              </p>
            </div>
          )}
            ...
            <div className="mb-12 w-full max-w-2xl animate-in fade-in duration-300">
              <div className="flex items-end justify-center gap-1.5 h-20 mb-10">
                {[...Array(24)].map((_, i) => (
                  <div
                    key={i}
                    className="w-1.5 bg-teal-400 rounded-full animate-voice-pulse"
          ...
          {!processing && (
            ...
            >
              {isRecording ? (
                ...
                <Mic
                  className={cn(
                    "w-12 h-12 text-white group-hover:scale-110 transition-transform",
                    !isRecording &&
          ...
          {!isRecording && (
            <form
              ...
              className="mt-12 w-full max-w-xl group relative animate-in slide-in-from-bottom-4 duration-500"
            >
              <input
                type="text"
                value={transcript}
        ...
        {sessions.length === 0 ? (
          <Card className="p-12 text-center border-dashed border-white/10 bg-white/[0.02]">
            <p className="text-white/30 font-medium">
              No voice tutoring history found.
            </p>
          </Card>
        ) : (
          <div className="grid gap-6">
            {sessions.map((s) => (
              <Card
                key={s._id}
                className="p-0 border-white/5 overflow-hidden hover:border-white/10 transition-colors"
              >
                <div className="p-6 md:p-8 flex flex-col md:flex-row gap-8">
                  <div className="flex-1 space-y-4">
                    ...
                    <h3 className="text-xl font-bold text-white leading-tight underline decoration-white/0 decoration-2 underline-offset-4 group-hover:decoration-amber-500/50 transition-all">
                      "{s.transcript}"
                    </h3>
                  ...
                  <div className="flex md:flex-col gap-3 justify-center min-w-[160px]">
                    {speaking === s._id ? (
                      <Button
                        onClick={stopAudio}
                        variant="destructive"
                        className="w-full h-12 flex items-center gap-2 shadow-lg shadow-rose-500/20"
                      >
                      ...
                      <Button
                        onClick={() =>
                          playAudio(s.aiResponse, s.language, s._id)
                        }
                        variant="secondary"
                        className="w-full h-12 flex items-center gap-2 bg-white/5 border-white/10 hover:bg-white/10"
                      >
                    ...
                    <Button
                      ...
                      className={cn(
                        ...
                        s.savedToQA
                          ...
                          : "text-teal-400 border-teal-500/20 hover:bg-teal-500/10 hover:border-teal-500/30",
                      )}
                    >
                      <Save className="w-4 h-4" />{" "}
                      {s.savedToQA ? "Shared to Q&A" : "Share Feature"}
                    </Button>
                  </div>
                </div>
              </Card>
